...
namespace StudentManagementSystem.BL
{
    public class StudentMarkBL
    {
        ...
        public List<StudentMarkCustomDTO> SearchStudentMark(string SearchText)
        {
            ...
            {
                sb.Clear();
                sb.Append("SELECT sub.SubjectName,stm.StudentId,stm.Marks FROM dbo.Subject AS sub RIGHT JOIN dbo.StudentMark AS stm ON sub.SubjectId = stm.SubjectId WHERE stm.StudentId LIKE '%' + ?StudentId + '%'");
