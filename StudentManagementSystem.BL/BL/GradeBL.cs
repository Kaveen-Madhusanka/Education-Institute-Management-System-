...
namespace StudentManagementSystem.BL
{
    public class GradeBL
    {
        ...
        public List<GradeDTO> filterBySubjectId(string subjectId)
        {
            List<GradeDTO> oGradeList = new List<GradeDTO>();
            try
            {
                sb.Clear();
                sb.Append("SELECT GradeId, Description, LowerLimit, UpperLimit, SubjectId");
                sb.Append(" FROM [Grade]");
                sb.Append(" WHERE SubjectId LIKE '%' + ?SubjectId + '%'");
